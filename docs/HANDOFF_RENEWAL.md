...
> **2026-09-07 카피 수정 승인:** 대표의 후속 요청 “전체적으로 텍스트가 많으니 카피를 미니멀하게 바꾸고 변호사들이 이해하기 쉽게 수정”에 따라 기존 카피 잠금을 해제했다. D 키네틱·블루/화이트는 유지하고, 홈·서비스·문의의 반복 설명과 전문용어를 축약했다. 가격·실제 사실·제공 조건·기술 불변 조건은 계속 보호한다. 상세 변경 및 검증은 [RENEWAL_MINIMAL_COPY.md](RENEWAL_MINIMAL_COPY.md). main 미병합.

> **2026-09-07 디자인 브랜치 추가:** 대표가 D(키네틱)를 선택하고 명함의 `#004AAD`/화이트를 지정했다. `design/renewal-v2`에서 홈과 하위 페이지에 적용했으며 **main 미병합**이다. 상세 범위·검증·대표 결정 항목은 [RENEWAL_KINETIC_ROLLOUT.md](RENEWAL_KINETIC_ROLLOUT.md). 아래의 8월 인수인계는 기존 main 기준 기록이다. 새 디자인 방향은 대표의 최신 지시를 따르되, 카피·사실·기술 불변 조건은 그대로 유지한다.
