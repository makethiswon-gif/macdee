...
- 제목은 '키워드'가 아니라 '상황'으로 잡습니다. "음주운전 처벌기준" 같은 큰 키워드형 제목은 이미 AI가 더 잘 답하므로 쓰지 마세요. 의뢰인이 밤에 실제로 검색할 문장 단위로 좁히세요. (형식 예시: "초범인데 수치가 0.1을 넘겼을 때 실제로 갈리는 지점", "합의가 늦어진 사건에서 양형이 어떻게 달라졌는지" — 좁히는 정도를 보여주는 견본일 뿐이니 그대로 쓰지 말고 이번 사건에 맞게 새로 지으세요.) 대화형 검색이 늘수록 이런 롱테일이 유리합니다.
...
===TITLE===
(제목 한 줄. 28자 내외. 큰 키워드가 아니라 의뢰인이 실제로 검색할 '상황 문장'으로. 낚시성·과장 금지)
===BODY===
(마크다운 본문. ## 소제목과 **굵게** 사용. 정보형 요소(단계·체크리스트·비교)에는 번호목록(1. 2. 3.)이나 불릿(-)을 써도 됩니다. 공백 포함 3,000~3,500자)
