...
export async function POST(request: Request) {
    try {
        ...
        if (urls.length > 10) {
            return NextResponse.json({ error: "URL은 최대 10개까지만 분석 가능합니다." }, { status: 400 });
        }
        ...
        if (validResults.length === 0) {
            return NextResponse.json({ error: "입력하신 URL에서 충분한 텍스트를 추출하지 못했습니다. 네이버 블로그, 브런치 등 텍스트가 많은 게시글 링크를 입력해주세요." }, { status: 400 });
        }

        console.log(`[Tone Analysis] Successfully scraped ${validResults.length}/${urls.length} URLs.`);

        // Combine text with clear separation
        const combinedText = validResults.map((res, i) => `[샘플 글 ${i + 1} - ${res?.title}]\n${res?.text}\n\n`).join("---\n\n");
        const totalLength = combinedText.length;

        console.log(`[Tone Analysis] Combined text length: ${totalLength} chars.`);

        if (totalLength < 300) {
            return NextResponse.json({ error: "의미 있는 문체를 추출하기에는 추출된 텍스트량이 너무 적습니다. 더 긴 글이 포함된 URL을 추가해주세요." }, { status: 400 });
        }

        // 4. Extract Tone using AI (Claude)
        const generator = getContentGenerator();

        const SYSTEM_PROMPT = `당신은 작가의 문체(Tone of Voice)를 분석하고 모방 규칙을 추출하는 '문체 분석 전문가'입니다.
