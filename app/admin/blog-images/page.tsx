        ...
        <div className="grid items-start gap-7 xl:grid-cols-[350px_minmax(0,1fr)]">
            <fieldset disabled={busy || loading} className="min-w-0 space-y-5 rounded-xl border border-slate-800 bg-slate-900 p-5">
                ...
                {selectedProfile && (() => { const idn = getMagazineIdentity(selectedProfile);
                    const pal = { cobalt: "코발트", vermilion: "버밀리언", forest: "포레스트", aubergine: "오베르진", graphite: "그래파이트", amber: "앰버", burgundy: "버건디", teal: "틸", slate: "슬레이트", olive: "올리브" }[idn.palette];
                    return <p className="text-xs text-slate-400">이 변호사의 고정 지면: <span className="text-emerald-300">{pal} · {idn.typography === "serif" ? "명조" : "고딕"} · {idn.style === "contrast" ? "어두운 지면" : "밝은 지면"}</span> — 골격·구도·장면은 원고마다 새로 조판됩니다(같은 원고는 항상 같게). 지면 색이 다른 변호사와 겹치면 블로그 설정의 DNA 조정칸으로 갈라냅니다.</p>; })()}
