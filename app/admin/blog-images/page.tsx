        ...
        <div className="grid items-start gap-7 xl:grid-cols-[350px_minmax(0,1fr)]">
            <fieldset disabled={busy || loading} className="min-w-0 space-y-5 rounded-xl border border-slate-800 bg-slate-900 p-5">
                ...
                {selectedProfile && (() => { const idn = getMagazineIdentity(selectedProfile);
                    return <p className="text-xs text-slate-400">이 변호사의 고정: <span className="text-emerald-300">{idn.palette} · {({ journal: "저널형", poster: "포스터형", column: "컬럼형" } as const)[idn.family]} · {idn.typography === "serif" ? "명조" : "고딕"} · {idn.style === "contrast" ? "어두운 지면" : "밝은 지면"}</span> — 색·형식은 변호사마다 다르게 고정되고, 장면·미세 조판은 원고마다 새로 나옵니다(같은 원고는 항상 같게).</p>; })()}
