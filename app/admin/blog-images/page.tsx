                ...
                : <div className="grid items-start gap-6 md:grid-cols-2">{ordered.map((type) => {
                    ...
                    return <article key={type} aria-label={CARD_LABELS[type] + " 결과"} className="min-w-0 overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
                        ...
                        <div className="space-y-3 p-4">
                            ...
                            {card && <details className="border-t border-slate-800 pt-3 text-sm"><summary className="cursor-pointer text-slate-300">제목·레이아웃 편집</summary><div className="mt-3 space-y-3">
                                <label className="block text-xs text-slate-400">이미지 제목<input aria-label={card.name + " 제목 수정"} disabled={busy} maxLength={70} value={headingEdits[type] ?? frozen?.plan.cards.find((c) => c.type === type)?.heading ?? ""} onChange={(e) => setHeadingEdits((prev) => ({ ...prev, [type]: e.target.value }))} className={inputClass + " mt-2"} /></label>
                                <div className="flex flex-wrap gap-3"><button disabled={busy} onClick={() => void generate(type, true)} className="rounded-md border border-slate-600 px-3 py-2 text-xs disabled:opacity-40">제목 적용</button><button disabled={busy} onClick={() => void generate(type, true, card.layout === "paper" ? "contrast" : "paper")} className="rounded-md border border-slate-600 px-3 py-2 text-xs disabled:opacity-40">다른 레이아웃</button></div>
