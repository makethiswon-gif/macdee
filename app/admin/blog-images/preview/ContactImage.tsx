"use client";
import { S, FONT, TS, CL_ALL, getContrastColor, getSubContrastColor } from "./designs";
...
export default function ContactImage({ config, profile }: P) {
    ...
    if (v >= 23 && v <= 39) { // Remaining variants: simple layout variations
        const variants: React.CSSProperties[] = [
            { background: `linear-gradient(180deg,${accent}40,${bg})` }, // 23
            { background: bg }, // 24
            { background: `linear-gradient(135deg,${bg},${accent}25)` }, // 25
            { background: `linear-gradient(180deg,${bg},${a2}30)` }, // 26
            { background: `${accent}` }, // 27
            { background: `linear-gradient(160deg,${accent},${a2})` }, // 28
            { background: bg }, // 29
            { background: `linear-gradient(135deg,${a2},${accent})` }, // 30
            { background: bg }, // 31
            { background: `linear-gradient(180deg,${accent}30,${bg})` }, // 32
            { background: bg }, // 33
            { background: `linear-gradient(135deg,${bg},${a2}40)` }, // 34
            { background: bg }, // 35
            { background: `linear-gradient(180deg,${accent},${bg})` }, // 36
            { background: bg }, // 37
        ];
        const idx = v - 23;
        const bgStyle = variants[idx] || { background: bg };
        const isAccentBg = v === 27 || v === 28 || v === 30;
        const fgColor = isAccentBg ? getContrastColor(accent) : tc;
        const subColor = isAccentBg ? getSubContrastColor(accent) : `${tc}AA`;
        const acColor = isAccentBg ? `${fgColor}40` : accent;

        return <div id="blog-contact-image" style={{ ...base, ...bgStyle }}>
            {/* Top accent strip for some variants */}
            {(v === 24 || v === 29 || v === 31 || v === 33 || v === 35 || v === 37) && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: v % 2 === 0 ? 8 : 6, background: accent }} />}
            {/* Photo bg for some variants */}
            {(v === 26 || v === 32 || v === 34 || v === 36) && oImg && <><div style={{ position: "absolute", inset: 0, backgroundImage: `url(${oImg})`, backgroundSize: "cover", opacity: 0.1 }} /><div style={{ position: "absolute", inset: 0, background: `linear-gradient(180deg,${bg}EE,${bg}CC)` }} /></>}
            <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: v % 3 === 0 ? "row" : "column", alignItems: v % 3 === 0 ? "center" : (v % 2 === 0 ? "center" : "flex-start"), justifyContent: "center", padding: v % 3 === 0 ? "50px 60px" : "50px 70px", gap: v % 3 === 0 ? 40 : 0, textAlign: v % 2 === 0 && v % 3 !== 0 ? "center" : "left" }}>
                {/* Profile photo */}
                {v % 3 === 0 && <div style={{ flex: "0 0 auto" }}>{profImg(140, 170, 14) || <div style={{ width: 140, height: 170, borderRadius: 14, background: `${acColor}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48, fontWeight: 800, color: fgColor }}>{nm[0]}</div>}</div>}
                {v % 2 === 0 && v % 3 !== 0 && profImg(70, 70, 35) && <div style={{ marginBottom: 16 }}>{profImg(70, 70, 35)}</div>}
                <div>
                    <p style={{ color: acColor, fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", marginBottom: 12 }}>상담 안내</p>
                    <h2 style={{ color: fgColor, fontSize: 32, fontWeight: 900, marginBottom: 4 }}>{nm} 변호사</h2>
                    {of && <p style={{ color: subColor, fontSize: 14, marginBottom: 24 }}>{of}</p>}
                    <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 8 }}>
                        {ci.map((c, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <div style={{ width: 40, height: 40, borderRadius: 10, background: `${acColor}20`, display: "flex", alignItems: "center", justifyContent: "center", color: acColor }}>{c.icon}</div>
                            <div><p style={{ color: subColor, fontSize: 11 }}>{c.label}</p><p style={{ color: fgColor, fontSize: 15, fontWeight: 600 }}>{c.value}</p></div>
                        </div>)}
                    </div>
                </div>
            </div>{logoEl}
