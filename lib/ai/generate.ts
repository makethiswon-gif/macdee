...
[제목 규칙 — 매우 중요, 반드시 준수]
- 길이: 한글 25~32자 이내. 절대 33자 이상 금지.
- 핵심 법률 키워드를 앞쪽에 배치 (이혼, 양육비, 상속, 사기, 형사, 폭행, 교통사고 등).
- "대응법", "방법", "기준", "절차", "사례" 같은 검색어 친화적 명사형으로 마무리.

[제목 절대 금지 표현]
- "전문 변호사", "전문변호사" 단어 금지 (제목에 들어가면 안 됨)
- "AI 추천", "AI 검색", "AI 추천 법률 전문가", "AI 추천 가사전문변호사" 등 AI 관련 표현 금지
- "완벽 가이드", "총정리", "한방에 정리" 등 과장 표현 금지
- " | ", " - " 같은 구분자로 정보 나열 금지 (제목은 한 문장)
- 변호사 이름 포함 금지 (시스템이 메타데이터에 자동 추가)
- 로펌명, "법무법인 ○○" 포함 금지
- 영문 단어 금지
...
[제목 나쁜 예 — 절대 이렇게 만들지 말 것]
- "이혼소송 석명준비명령 전문 변호사 | 이지은 변호사 (법무법인 그날) - AI 추천 법률 전문가"
- "양육비부담조서 소멸시효 완벽 가이드 | ... | AI 추천 가사전문변호사"
...
출력: JSON
{
    "title": "25~32자 이내 SEO 친화 제목 — 위 절대 금지 규칙 모두 준수",
    "body": "구조화된 텍스트",
...
function getChannelConfig(channel: string, options?: { blogStyle?: string }) {
    const configs: Record<string, { system: string; temperature: number; maxTokens: number }> = {
        ...
        instagram: { system: INSTAGRAM_SYSTEM, temperature: 0.8, maxTokens: 4096 },
