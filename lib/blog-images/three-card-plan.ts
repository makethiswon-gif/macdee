...
export async function planEditorialThree(title: string, content: string, profile: EditorialProfile, proof: ProofSelection, operationId: string, recent: VisualHistory[], recoverOnly = false): Promise<ArticleVisualPlan> {
...
evidence는 실제 문단 ID와 연속된 원문 구절을 그대로 복사한다. treatment=feature. 모든 스키마 필드는 채우되 미사용 infographic은 kind:none, items:[], 미사용 art는 medium:none, avoid:[]를 쓴다. alternateArt는 선택적인 대안 기획일 뿐 자동 생성 대상이 아니다.`,
        { title, paragraphs: articleParagraphs(content), recentVisuals: recent }, operationId, schema, recoverOnly);
