...
(async () => {
    ...
    assert.ok($('#records').text().includes('월 1회 모아서 올려도 됩니다.'));
    assert.ok($('#records').text().includes('식별정보는 빼고'));
    assert.ok($('#records').text().includes('광고 검색어·예산·콘텐츠 주제'));
    assert.ok($('main').text().includes('블로그 재가공뿐 아니라'));
    assert.ok(!$('main').text().includes('블로그 콘텐츠 재구성 중심'));
    ...
    for (const width of [1440, 375]) {
        ...
        await p.locator('#records').screenshot({ path: path.join(out, `records-flow-${width}.png`) });
