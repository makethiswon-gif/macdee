...
(async () => {
    ...
    const { STANDARD_OFFER, GROWTH_OFFER, UPGRADE_SERVICES, UPGRADE_FAQ } = data('data/renewal/upgrade.ts');
    ...
    assert.deepEqual(now.PLANS.slice(2), old.PLANS.slice(2), 'MARKET LEADER unchanged');
    assert.equal(now.PLANS[1].price, '월 500만원');
    assert.equal(GROWTH_OFFER.videoPosts, 20); assert.equal(GROWTH_OFFER.shortsPosts, 12);
    assert.deepEqual(now.PLANS[1].includes.slice(2), old.PLANS[1].includes, 'Existing GROWTH scope retained');
    assert.deepEqual(now.PLANS[1].includes.slice(0,2), GROWTH_OFFER.videoIncludes);
    ...
    report.facts.push('STANDARD 250만원 / GROWTH 500만원 with 20 videos including 12 Shorts and filming/editing / ad spend separate / plan inheritance');
    ...
    if(!production) assert.equal(git(['diff', baseline, '--', ...protectedPaths]), '', 'Protected site/API/customer areas unchanged');
    else assert.equal(git(['diff', '00f3662', '--', 'app/api', 'app/admin', 'app/(dashboard)', 'app/(auth)', 'app/portal', 'app/diagnose', 'app/blog', 'app/site', 'lib', 'middleware.ts', 'package.json', 'package-lock.json']), '', 'Latest production product/admin/API unchanged');
    const raw = execFileSync('curl.exe', ['-sS', '--max-time', '60', '-A', 'ChatGPT-User', origin + (production ? '/upgrade' : '/renewal/upgrade')], { encoding: 'utf8', maxBuffer: 20 * 1024 * 1024 });
    const $ = load(raw);
    report.page.canonical = $('link[rel=canonical]').attr('href');
    assert.equal(report.page.canonical, 'https://www.makethis1.com' + (production ? '/upgrade' : '/renewal/upgrade'));
    ...
    for (const width of [1440, 375]) {
        ...
        const growthCard = p.locator('.mt-plan').nth(1);
        for (const text of [GROWTH_OFFER.price, ...GROWTH_OFFER.videoIncludes]) assert.ok((await growthCard.textContent()).includes(text));
        await card.scrollIntoViewIfNeeded();
        await p.screenshot({ path: path.join(out, `home-plan-${width}.png`) });
        await p.locator('.mt-upgrade-link').click();
        await p.waitForURL(production ? '**/upgrade' : '**/renewal/upgrade');
        ...
        await p.waitForURL(production ? '**/upgrade#included' : '**/renewal/upgrade#included');
        ...
        await p.waitForURL(production ? '**/consult?plan=standard#form' : '**/diagnose?plan=standard#form');
        ...
        await p.goto(origin + '/renewal/upgrade', { waitUntil: 'networkidle' });
        await p.getByRole('link', { name: 'GROWTH 상담하기' }).click();
        await p.waitForURL(production ? '**/consult?plan=growth#form' : '**/diagnose?plan=growth#form');
        await p.waitForFunction(() => document.querySelector('#plan')?.value === 'growth');
        assert.ok((await p.locator('#plan option:checked').textContent()).includes('월 500만원'));
        await p.locator('#firmName').fill('QA 모의 로펌'); await p.locator('#contactName').fill('QA'); await p.locator('#phone').fill('000-0000-0000');
        await p.getByRole('button', { name: '상담 요청 보내기' }).click(); await p.getByRole('status').waitFor();
        assert.equal(requests, 2); assert.equal(payload.plan, 'growth'); assert.ok(payload.note.includes('월 500만원'));
        report.forms.push({ width, upgradeToGrowth500: true, selectedOfferPreserved: true, mockedRequests: 1 });
        await context.close();
    }
    await browser.close();
